def numbers_to_words(x)
    ...
    if length == 3
        ...
        puts "Sorry Chale, number between 0 and 100 only"
    end
end
...
def n_to_w(x)
    ...
    unit_text = ""
    if length == 4
        billion_text = numbers_to_words(outer_array[0]) + " billion"
        million_text = numbers_to_words(outer_array[1]) + " million"
        thousand_text = numbers_to_words(outer_array[2])
        thousand_text.strip.empty? ? thousand_text = thousand_text.strip : thousand_text = thousand_text.strip + " thousand"
        unit_text = numbers_to_words(outer_array[3])

        output = "#{billion_text}, #{million_text}, #{thousand_text}, #{unit_text}"
    elsif length == 3
        million_text = numbers_to_words(outer_array[0]) + " million"
        thousand_text = numbers_to_words(outer_array[1])
        thousand_text.strip.empty? ? thousand_text = thousand_text.strip : thousand_text = thousand_text.strip + " thousand"
        unit_text = numbers_to_words(outer_array[2])
        output = "#{million_text}, #{thousand_text}, #{unit_text}"
    elsif length == 2
        thousand_text = numbers_to_words(outer_array[0]) + " thousand"
        unit_text = numbers_to_words(outer_array[1])
        output = "#{thousand_text}, #{unit_text}"
    elsif length == 1
        unit_text = numbers_to_words(outer_array[0])
        output = "#{unit_text}"
    else
